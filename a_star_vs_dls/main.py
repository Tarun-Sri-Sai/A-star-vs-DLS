...
def run_logs(file_in, logs_writer, paths_dir):
    ...
    if dls_success:
        print('Path found using Depth Limited search!\t',
              end='', file=logs_writer)
    ...
    print('\n', '=' * 30, ' Algorithm Analysis ',
          '=' * 30, sep='', file=logs_writer)
    print(
        f'\nA-star search took\t\t\t\t{end_time_a_star:3.2f} microseconds', file=logs_writer)
    print(
        f'Depth Limited search took\t\t\t{end_time_dls:3.2f} microseconds', file=logs_writer)
    print('\n', '=' * 80, sep='', file=logs_writer)

    diff = end_time_dls - end_time_a_star
    if diff < 0:
        print(
            f'Depth Limited search was faster by\t\t{-diff:3.2f} microseconds', file=logs_writer)
    elif diff > 0:
        print(
            f'A-star search was faster by\t\t\t{diff:3.2f} microseconds', file=logs_writer)
    else:
    ...
    print(
        f'Depth Limited search visits\t\t\t{graph.dls_count}', file=logs_writer)
    print('\n', '=' * 80, sep='', file=logs_writer)

    diff = graph.a_star_count - graph.dls_count
    if diff < 0:
        print(
            f'A-star search beats Depth Limited search by\t{-diff} visits', file=logs_writer)
    else:
        print(
            f'Depth Limited search beats A-star search by\t{diff} visits', file=logs_writer)
    print('=' * 80, file=logs_writer)
    ...
    print(
        f'Depth Limited search found a path that costs\t{dls_cost}', file=logs_writer)
    print('\n', '=' * 80, sep='', file=logs_writer)

    diff = a_star_cost - dls_cost
    if diff < 0:
        print(
            f'A-star search found a path cheaper by\t\t{-diff}', file=logs_writer)
    elif diff > 0:
        print(
            f'Depth Limited search found a path cheaper by\t{diff}', file=logs_writer)
    else:
